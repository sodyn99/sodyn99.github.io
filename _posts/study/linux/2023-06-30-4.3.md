...
# 2. ns-3 다운로드
ns-3.33 버전 문서<sup>[[1]](#1)</sup>를 바탕으로 진행.
...
- task in 'ns3-core' failed with exit status 1: (csv-reader.cc 파일 오류)
  ...
  | 오류  | 해결  |
  | :---: | :---: |
  | error: ‘numeric_limits’ is not a member of ‘std’  | csv-reader.h 파일에<br>`#include <stddef.h>`<br>`#include <limits>`<br>를 포함시킨다.<sup>[[2]](#2)</sup><br>  `$ vi ./src/core/helper/csv-reader.h` |
...
<img src="/images/study/linux/4.2-1.png" alt="build finished" class="gallery-img" width="50%" height="50%">

**기본 예제 연습을 위해서 [4.4 예제](/study/computer/linux/4.4)로 이동.**

\* ns-3 최신 버전에서는 `waf`가 `ns3`로 변경되었다.
   <div markdown="1" style="float:left;width:47.5%;">
   ```bash
   $ ./waf configure
   $ ./waf build
   $ ./waf --run
   ...
   <div style="display:flex;align-items:center;justify-content:center;float:left;width:5%;height:6.5em;">&rarr;</div>
   <div markdown="1" style="float:left;width:47.5%;">
   ```bash
   $ ./ns3 configure
   $ ./ns3 build
...
[<a name="1">1</a>] [ns-3.33 문서](https://www.nsnam.org/releases/ns-3-33/documentation/){:target="_blank"}

[<a name="2">2</a>] [‘numeric_limits’ is not a member of ‘std’](https://stackoverflow.com/questions/71296302/numeric-limits-is-not-a-member-of-std){:target="_blank"}, stackoverflow.