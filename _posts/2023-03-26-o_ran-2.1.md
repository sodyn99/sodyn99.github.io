...
1. Experimental Scenario
  ...
  2. UE가 네트워크의 한 slice에 정적으로 할당되는 multi-slice 시나리오를 고려  
  ...
  3. SrsLTE(5G NR에 대한 새로운 초점 반영을 위해 srsRAN으로 이름이 변경됨)를 사용하여 소프트웨어화 셀룰러 네트워크를 구현함 → SrsLTE는 경량 코어 네트워크와 BS 및 UE의 full-stack 구현을 제공하는 오픈 소스 프레임워크  
