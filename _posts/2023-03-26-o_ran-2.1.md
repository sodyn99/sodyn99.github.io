...
3. 배치 및 인터페이스
    - 분해 배치의 한 예 “시나리오 B”
        ...
        <img src="/images/o_ran-2.1/architecture.png" width="70%" height="70%">
...
1. 데이터 처리 및 교육 절차
    ...
    - 1. AI/ML 모델은 잘 구축된 NFV-MANO 아키텍처와 윳한 마켓플레이스 시스템을 통해 운영자에게 제공
        모델은 제어 목표, 리소스, 예상 입출력 세부 정보와 함께 카탈로그에 저장
    - 2. 데이터 기반 솔루션은 오프라인에서 교육 및 검증 → 비효율성 또는 운영 중단을 방지
        모든 ML 모델을 오라인에서 교육, 검증 받아야함  
...
2. 제어 루프
    ...
    아래 그림은 지능(inteligence)가 다른 레이어에 어떻게 포함될 수 있는지, 세분화된 셀룰러 네트워크의 실체 및 이에 대한 도전과 한계를 보여준다.
